...
"""
TO DO LIST
- Tide Height data per hour closest data point (interpolate?)
...
forecast = {}
page = requests.get("https://www.windfinder.com/forecast/weston_southampton")
...
for a in range(1, 11):  # iterate over each days forecast
    forecastContainer = soup.find_all(class_="weathertable forecast-day forecast forecast-day-8 fc-day-index-" + str(a))
    ...
    for dailyForecast in forecastContainer:
        ...
        direction = getData(dailyForecast, "data-direction-unit units-wd units-wd-deg data--minor weathertable__cell")
...
for day in forecast:
    ...
    for indexTide, tideTime in enumerate(tideTimes):
        if tideType[indexTide] == "high":
            ...
            timeEarly = time - datetime.timedelta(hours=4)
            if timeEarly < datetime.datetime(1900, 1, 1):
                timeEarly = datetime.datetime(1900, 1, 1, 0, 0)
            timeLate = time + datetime.timedelta(hours=4)
            ...
            if datetime.datetime(1, 1, 1, 9).time() < timeEarly.time() < datetime.datetime(1, 1, 1, 17).time() or datetime.datetime(1, 1, 1, 9).time() < timeLate.time() < datetime.datetime(1, 1, 1, 17).time():
                ...
                if sessionLength > datetime.timedelta(hours=1.5):
                    ...
                    if 6 < baseHigh < 22:
                        print(day, ":", timeEarly.strftime('%H:%M'), "-", timeLate.strftime('%H:%M'))
                        print("Session Length", ":".join(str(sessionLength).split(":")[:2]))
                        ...
                        if baseChange == "increasing":
                            if gustChange == "increasing" or gustChange == "constant":
                                print("Forecast: ", baseLow, "gusting", gustLow, "increasing to", baseHigh, "gusting",
                                      gustHigh)
                            else:
                                print("Forecast: ", base[0], "gusting", gust[0], "changing to", base[len(base) - 1],
                            ...
                            if gustChange == "increasing" or gustChange == "increase decrease":
                                ...
                                print("Forecast:", baseLow, "gusting", gustLow)
                            elif gustChange == "decrease increase":
                                print("Forecast: ", baseLow, "gusting", gustLow,
                                      "decreasing slightly then increasing to", baseHigh, "gusting", gustHigh)
                            else:
                                print("Forecast:", baseLow, "gusting", gustLow)
                        elif baseChange == "decreasing":
                            if gustChange == "decreasing" or gustChange == "constant":
                                print("Forecast:", baseHigh, "gusting", gustHigh, "decreasing to", baseLow, "gusting",
                                      gustLow)
                            else:
                                print("Forecast: ", base[0], "gusting", gust[0], "changing to", base[len(base) - 1],
                                      "gusting", gust[len(base) - 1])
                            ...
                            print("Forecast: ", baseLow, "gusting", gustLow,
                                  "increasing slightly to", baseHigh, "gusting", gustHigh, "and then decreasing again")
